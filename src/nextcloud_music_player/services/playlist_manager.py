...
class PlaylistManager:
    ...
    def create_default_playlist_if_needed(self) -> Dict[str, Any]:
        ...
        if current_playlist is None:
            ...
            if existing_playlists:
                ...
                logger.info(
                    f"使用现有播放列表作为当前播放列表: {first_playlist['name']}"
                )
            else:
                # 创建新的默认播放列表
    ...
    def create_empty_playlist(
        self, name: str = "新播放列表", folder_path: str = ""
    ) -> Dict[str, Any]:
        """创建空的播放列表"""
        playlists_data = self.load_playlists_data()
    ...
    def create_playlist_from_folder(
        self, folder_path: str, name: str = None
    ) -> Dict[str, Any]:
        """从文件夹创建播放列表"""
        if not name:
        ...
        logger.info(
            f"从文件夹创建播放列表: {name} (ID: {next_id}, 歌曲数: {len(songs)})"
        )
        return new_playlist
    ...
    def add_song_to_current_playlist(self, song_info: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            song_entry = {
                "name": song_name,
                "info": song_info,
                "state": {"play_count": 0, "is_favorite": False, "last_played": None},
            }
    ...
    def add_songs_to_current_playlist_batch(
        self, song_infos: List[Dict[str, Any]]
    ) -> int:
    ...
    def get_playlist_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"total_songs": 0, "current_index": 0, "playlist_name": "错误"}

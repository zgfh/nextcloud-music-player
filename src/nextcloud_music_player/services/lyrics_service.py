...
class LyricsService:
    ...
    def get_remote_lyrics_path(
        self, song_name: str, song_remote_path: str = None
    ) -> str:
        """
    ...
    async def download_lyrics(
        ...
        try:
            ...
            # 获取远程歌词文件路径
            remote_lyrics_path = self.get_remote_lyrics_path(
                song_name, song_remote_path
            )
            ...
            # 检查远程文件是否存在
            try:
                file_info = await self.nextcloud_client.get_file_info(
                    remote_lyrics_path
                )
                if not file_info:
            ...
            success = await self.nextcloud_client.download_file(
                remote_lyrics_path, local_lyrics_filename, local_lyrics_path
            )
        ...
        try:
            ...
            for line in lines:
                ...
                if (
                    line.startswith("[")
                    and ":" in line
                    and not self.time_pattern.match(line)
                ):
                    try:
                ...
                if time_matches:
                    ...
                    for time_match in time_matches:
                        try:
                            ...
                            # 转换为总秒数
                            total_seconds = (
                                minutes * 60 + seconds + centiseconds / 100.0
                            )
            ...
            logger.info(
                f"成功解析歌词: {len(lyrics_lines)} 行歌词, {len(metadata)} 个元数据"
            )
            ...
            if auto_download and self.nextcloud_client:
                ...
                if loop.is_running():
                    ...
                    task = loop.create_task(
                        self._download_and_load_lyrics(song_name, song_remote_path)
                    )
    ...
    async def _download_and_load_lyrics(
        self, song_name: str, song_remote_path: str = None
    ):
        """下载并加载歌词的异步方法"""
        try:
    ...
    def get_lyrics_around_position(
        self, position_seconds: float, context_lines: int = 2
    ) -> List[LyricLine]:
        """
    ...
    def has_lyrics(self, song_name: str = None) -> bool:
        ...
        if song_name:
            ...
            return self.current_lyrics is not None and bool(
                self.current_lyrics.get("lines")
            )

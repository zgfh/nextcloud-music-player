...
class CodeQualityTool(BaseTool):
    ...
    def _check_security_patterns(self, content: str, file_path: str) -> list:
        ...
        security_patterns = [
            ...
            ("input(", "Use of input() should be validated"),
    ...
    def _check_type_patterns(self, content: str, file_path: str) -> list:
        """Check for type-related issues."""
        issues = []
        lines = content.split('\\n')
        
        type_patterns = [
            ("def ", "Function missing type hints"),
            ("return", "Return statement may need type annotation")
        ]
        
        for line_num, line in enumerate(lines, 1):
            stripped_line = line.strip()
            if stripped_line.startswith("def ") and "->" not in line:
                issues.append({
                    "file": file_path,
                    "line": line_num,
    ...
    def _basic_format_python(self, content: str) -> str:

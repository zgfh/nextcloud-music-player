    ...
    async def _validate_env_file(self, content: str, result: dict) -> None:
        ...
        for line_num, line in enumerate(lines, 1):
            ...
            if line and not line.startswith('#'):
                if '=' not in line:
                    result["errors"].append(f"Line {line_num}: Invalid format, expected KEY=VALUE")
                else:
                    key, value = line.split('=', 1)
                    if not key.strip():
                        result["errors"].append(f"Line {line_num}: Empty key")
    
    async def _setup_logging_config(self, environment: str) -> None:
        """Setup logging configuration for environment."""
        log_config = {
            "version": 1,
        ...
        config_dir = self.project_root / "config"
        log_config_path = config_dir / f"logging_{environment}.json"
        
        self.write_file(log_config_path, json.dumps(log_config, indent=2))